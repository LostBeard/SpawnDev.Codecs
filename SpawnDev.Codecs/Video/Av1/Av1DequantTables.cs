...
//
// AV1 quantization lookup tables. Bit-exact port of libaom
// av1/common/quant_common.c dc_qlookup_QTX[] / ac_qlookup_QTX[]
// for 8 / 10 / 12 bit depth.
//
// Upstream Copyright (c) 2016, Alliance for Open Media.
// Upstream license: BSD 2-Clause + AV1 Patent License 1.0. See NOTICE.md.
//
// AV1 quantizers carry a Q3 scaling (3 bits over the input pixel scale)
// matching the +3 bits the forward transform pipeline adds to the
// coefficient magnitudes. For >256-pixel transforms the quant is
// descaled (>16x16 -> 2 bits, >32x32 -> 1 bit) so the dequantized output
// stays effectively Q3 across all transform sizes.
//
// Lookup formula (libaom <c>av1_dc_quant_QTX</c> / <c>av1_ac_quant_QTX</c>):
//   q_clamped = clamp(base_qindex + delta, 0, MAXQ);
//   return dc/ac_qlookup_{bd}_QTX[q_clamped];

namespace SpawnDev.Codecs.Video.Av1;

/// <summary>AV1 quantization lookup tables (qindex -> dequant value).</summary>
public static class Av1DequantTables
{
    /// <summary>libaom <c>QINDEX_RANGE</c>.</summary>
    public const int QindexRange = 256;

    /// <summary>libaom <c>MAXQ</c>.</summary>
    public const int MaxQ = 255;

    /// <summary>libaom <c>dc_qlookup_QTX[]</c> (8-bit input).</summary>
    public static readonly short[] DcLookup8 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>dc_qlookup_10_QTX[]</c> (10-bit input).</summary>
    public static readonly short[] DcLookup10 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>dc_qlookup_12_QTX[]</c> (12-bit input).</summary>
    public static readonly short[] DcLookup12 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>ac_qlookup_QTX[]</c> (8-bit input).</summary>
    public static readonly short[] AcLookup8 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>ac_qlookup_10_QTX[]</c> (10-bit input).</summary>
    public static readonly short[] AcLookup10 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>ac_qlookup_12_QTX[]</c> (12-bit input).</summary>
    public static readonly short[] AcLookup12 = new short[QindexRange]
    {
    ...
    /// <summary>libaom <c>av1_dc_quant_QTX</c>.</summary>
    public static short DcQuantQtx(int qindex, int delta, int bitDepth)
    {
        int qClamped = Math.Clamp(qindex + delta, 0, MaxQ);
        return bitDepth switch
        {
            8 => DcLookup8[qClamped],
            10 => DcLookup10[qClamped],
            12 => DcLookup12[qClamped],
            _ => throw new ArgumentOutOfRangeException(nameof(bitDepth), "must be 8, 10, or 12"),
        };
    }

    /// <summary>libaom <c>av1_ac_quant_QTX</c>.</summary>
    public static short AcQuantQtx(int qindex, int delta, int bitDepth)
    {
        int qClamped = Math.Clamp(qindex + delta, 0, MaxQ);
        return bitDepth switch
        {
            8 => AcLookup8[qClamped],
            10 => AcLookup10[qClamped],
            12 => AcLookup12[qClamped],
            _ => throw new ArgumentOutOfRangeException(nameof(bitDepth), "must be 8, 10, or 12"),
        };
    }
}
