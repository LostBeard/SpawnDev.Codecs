...
//
// 16x16 sibling of Vp9DcPredict4x4Kernel / Vp9DcPredict8x8Kernel.
// Same Vp9DcVariant routing and per-block-thread structure - only
// the per-edge sum extent (16 samples) and shift counts (Log2N = 4,
// Log2N + 1 = 5) change. Bit-exact against Vp9DcPredictor at N=16.
...
/// independent 16x16 blocks in parallel.
/// </summary>
public sealed class Vp9DcPredict16x16Kernel : IDisposable
{
    private const int N = 16;
    private const int Log2N = 4;

    private readonly Accelerator _accelerator;
    private readonly Action<Index1D, ArrayView<byte>, ArrayView<byte>, ArrayView<byte>, int, int, int> _kernel;

    /// <summary>Compile the kernel onto <paramref name="accelerator"/>.</summary>
    public Vp9DcPredict16x16Kernel(Accelerator accelerator)
    {
