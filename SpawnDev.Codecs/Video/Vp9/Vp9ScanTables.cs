...
//
// VP9 normative scan-order tables - bit-exact copy of libvpx
// vp9/common/vp9_scan.c.
//
// VP9 transmits coefficients in scan order: position 0 is always
// the DC coefficient, every subsequent position walks through the
// AC coefficients in an order chosen to put high-energy positions
// first. The scan table maps scan-position (0..N-1) to raster-
// position in the 2D block. The inverse-scan map is the entropy
// decoder's other direction.
//
// Three scan flavors at 4x4 / 8x8 / 16x16:
//   - default: a zigzag-style traversal optimized for DCT_DCT;
//     also reused for ADST_ADST.
//   - row:     column-leaning order optimized for ADST_DCT (rows
//     transformed by iADST concentrate coefficients near column 0,
//     so the scan walks down column 0 first, then column 1, ...).
//   - col:     row-leaning order, mirror of row scan, used for
//     DCT_ADST (cols transformed by iADST concentrate near row 0).
//
// Only one scan exists at 32x32 (default) - VP9 spec sec 8.5.2 and
// libvpx. The 32x32 transform never carries a non-DCT tx_type so a
// row/col variant isn't needed.
//
// Storage: raster positions fit in [0, 1023]. ushort is the smallest
// type wide enough for the 32x32 scan; ushort consistently across
// every table keeps the lookup math uniform regardless of size.

namespace SpawnDev.Codecs.Video.Vp9;

/// <summary>VP9 transform-block size enumeration (libvpx TX_SIZE).</summary>
public enum Vp9TxSize
{
    /// <summary>4x4 = 16 coefficients.</summary>
    Tx4x4 = 0,
    /// <summary>8x8 = 64 coefficients.</summary>
    Tx8x8 = 1,
    /// <summary>16x16 = 256 coefficients.</summary>
    Tx16x16 = 2,
    /// <summary>32x32 = 1024 coefficients.</summary>
    Tx32x32 = 3,
}

/// <summary>VP9 scan-order flavor for a given tx_type.</summary>
public enum Vp9ScanType
{
    /// <summary>Default zigzag (DCT_DCT and ADST_ADST at 4x4/8x8/16x16; only flavor at 32x32).</summary>
    Default = 0,
    /// <summary>Row scan (ADST_DCT).</summary>
    Row = 1,
    /// <summary>Column scan (DCT_ADST).</summary>
    Col = 2,
}

/// <summary>
/// VP9 scan-order tables (sec 8.5.2). All arrays are scan-position
/// to raster-position lookups: <c>raster_pos = scan[scan_pos]</c>.
/// </summary>
public static class Vp9ScanTables
{
    /// <summary>4x4 default scan (DCT_DCT, ADST_ADST). 16 entries.</summary>
    public static readonly ushort[] DefaultScan4x4 = new ushort[]
    {
        0, 4, 1, 5, 8, 2, 12, 9, 3, 6, 13, 10, 7, 14, 11, 15,
    };

    /// <summary>4x4 row scan (ADST_DCT). 16 entries.</summary>
    public static readonly ushort[] RowScan4x4 = new ushort[]
    {
        0, 1, 4, 2, 5, 3, 6, 8, 9, 7, 12, 10, 13, 11, 14, 15,
    };

    /// <summary>4x4 column scan (DCT_ADST). 16 entries.</summary>
    public static readonly ushort[] ColScan4x4 = new ushort[]
    {
        0, 4, 8, 1, 12, 5, 9, 2, 13, 6, 10, 3, 7, 14, 11, 15,
    };

    /// <summary>8x8 default scan (DCT_DCT, ADST_ADST). 64 entries.</summary>
    public static readonly ushort[] DefaultScan8x8 = new ushort[]
    {
    ...
    /// <summary>8x8 row scan (ADST_DCT). 64 entries.</summary>
    public static readonly ushort[] RowScan8x8 = new ushort[]
    {
    ...
    /// <summary>8x8 column scan (DCT_ADST). 64 entries.</summary>
    public static readonly ushort[] ColScan8x8 = new ushort[]
    {
    ...
    /// <summary>16x16 default scan (DCT_DCT, ADST_ADST). 256 entries.</summary>
    public static readonly ushort[] DefaultScan16x16 = new ushort[]
    {
    ...
    /// <summary>16x16 row scan (ADST_DCT). 256 entries.</summary>
    public static readonly ushort[] RowScan16x16 = new ushort[]
    {
    ...
    /// <summary>16x16 column scan (DCT_ADST). 256 entries.</summary>
    public static readonly ushort[] ColScan16x16 = new ushort[]
    {
        ...
        236, 159, 190, 221, 252, 175, 206, 237, 191, 253, 222, 238, 207, 254, 223, 239, 255,
    };

    /// <summary>32x32 default scan (only flavor at this size). 1024 entries.</summary>
    public static readonly ushort[] DefaultScan32x32 = new ushort[]
    {
    ...
    /// <summary>
    /// Pick the scan flavor for a given tx_type at 4x4 / 8x8 / 16x16
    /// (libvpx vp9_scan.h <c>get_scan</c>). Note that DCT_DCT and
    /// ADST_ADST share the default scan; the asymmetric tx_types
    /// 1 and 2 trigger row / col scans respectively.
    /// </summary>
    public static Vp9ScanType ScanTypeForTxType4x4(Vp9TxType4x4 txType) => txType switch
    {
        Vp9TxType4x4.DctDct   => Vp9ScanType.Default,
        Vp9TxType4x4.AdstDct  => Vp9ScanType.Row,
        Vp9TxType4x4.DctAdst  => Vp9ScanType.Col,
        Vp9TxType4x4.AdstAdst => Vp9ScanType.Default,
        _ => Vp9ScanType.Default,
    };

    /// <summary>Scan flavor for tx_type at 8x8.</summary>
    public static Vp9ScanType ScanTypeForTxType8x8(Vp9TxType8x8 txType) => txType switch
    {
        Vp9TxType8x8.DctDct   => Vp9ScanType.Default,
        Vp9TxType8x8.AdstDct  => Vp9ScanType.Row,
        Vp9TxType8x8.DctAdst  => Vp9ScanType.Col,
        Vp9TxType8x8.AdstAdst => Vp9ScanType.Default,
        _ => Vp9ScanType.Default,
    };
    ...
    /// the default table (libvpx never triggers those code paths at
    /// that size).
    /// </summary>
    public static ushort[] GetScan(Vp9TxSize txSize, Vp9ScanType scanType)
    {
        return txSize switch
        {
            Vp9TxSize.Tx4x4 => scanType switch
            {
                Vp9ScanType.Row => RowScan4x4,
                Vp9ScanType.Col => ColScan4x4,
                _ => DefaultScan4x4,
            },
            Vp9TxSize.Tx8x8 => scanType switch
            {
                Vp9ScanType.Row => RowScan8x8,
                Vp9ScanType.Col => ColScan8x8,
                _ => DefaultScan8x8,
            },
            Vp9TxSize.Tx16x16 => scanType switch
            {
                Vp9ScanType.Row => RowScan16x16,
                Vp9ScanType.Col => ColScan16x16,
                _ => DefaultScan16x16,
            },
            Vp9TxSize.Tx32x32 => DefaultScan32x32,
