// SpawnDev.Codecs is licensed under MIT (see LICENSE.txt).
//
// Structural port of libopus silk/table_LSF_cos.c to clean C#. Cosine lookup
// table used by silk_NLSF2A to convert normalized LSFs into 2*cos(LSF) via
// piecewise linear interpolation.
//
// Upstream Copyright (c) 2006-2011 Skype Limited. BSD 3-Clause. See NOTICE.md.

namespace SpawnDev.Codecs.Audio.Opus.Silk;

/// <summary>
/// 129-entry Q12 cosine approximation table for LSF-to-LPC conversion.
/// Index range: <c>[0, 128]</c>; the <c>128</c> sentinel is <c>-8192</c> (= -2.0 in Q12),
/// used to terminate the linear interpolation at the end of the table.
/// </summary>
internal static class SilkLsfCosTab
{
    /// <summary>
    /// <c>silk_LSFCosTab_FIX_Q12[i] = round(4096 * 2 * cos(pi * i / 128))</c> for
    /// <c>i in [0, 128]</c>. Range: <c>[-8192, 8192]</c> (i.e. <c>[-2, 2]</c> in Q12).
    /// </summary>
    internal static readonly short[] Q12 =
    {
        8192,  8190,  8182,  8170,  8152,  8130,  8104,  8072,
        8034,  7994,  7946,  7896,  7840,  7778,  7714,  7644,
        7568,  7490,  7406,  7318,  7226,  7128,  7026,  6922,
        6812,  6698,  6580,  6458,  6332,  6204,  6070,  5934,
        5792,  5648,  5502,  5352,  5198,  5040,  4880,  4718,
        4552,  4382,  4212,  4038,  3862,  3684,  3502,  3320,
        3136,  2948,  2760,  2570,  2378,  2186,  1990,  1794,
        1598,  1400,  1202,  1002,   802,   602,   402,   202,
           0,  -202,  -402,  -602,  -802, -1002, -1202, -1400,
       -1598, -1794, -1990, -2186, -2378, -2570, -2760, -2948,
       -3136, -3320, -3502, -3684, -3862, -4038, -4212, -4382,
       -4552, -4718, -4880, -5040, -5198, -5352, -5502, -5648,
       -5792, -5934, -6070, -6204, -6332, -6458, -6580, -6698,
       -6812, -6922, -7026, -7128, -7226, -7318, -7406, -7490,
       -7568, -7644, -7714, -7778, -7840, -7896, -7946, -7994,
       -8034, -8072, -8104, -8130, -8152, -8170, -8182, -8190,
       -8192
    };
}
