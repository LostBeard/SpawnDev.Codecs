// Tests for Av1ForwardAdst4Kernel. Validates bit-exact match with
// Av1ForwardAdst4.Transform across (a) zero (early-out path),
// (b) DC-only / structured, (c) random batches, (d) all cos_bit values.
...
public abstract partial class CodecsTestBase
{
    ...
    public async Task Av1ForwardAdst4Kernel_ZeroInput_MatchesCpuReference()
    {
        ...
        try
        {
            ...
            Av1ForwardAdst4.Transform(input, cpuOut);
    ...
    public async Task Av1ForwardAdst4Kernel_RandomBatch_MatchesCpuReference()
    {
        ...
        try
        {
            ...
            for (int t = 0; t < transformCount; t++)
                Av1ForwardAdst4.Transform(input.AsSpan(t * 4, 4), cpuOut.AsSpan(t * 4, 4));
    ...
    public async Task Av1ForwardAdst4Kernel_AllCosBits_MatchCpuReference()
    {
        ...
        try
        {
            ...
            for (int cosBit = 10; cosBit <= 13; cosBit++)
            {
                var cpuOut = new int[transformCount * 4];
                for (int t = 0; t < transformCount; t++)
                    Av1ForwardAdst4.Transform(input.AsSpan(t * 4, 4), cpuOut.AsSpan(t * 4, 4), cosBit);
