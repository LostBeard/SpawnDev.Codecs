// Tests for Vp9D117Predict8x8Kernel (slice 199).
...
{
    [TestMethod]
    public async Task Vp9D117Predict8x8Kernel_KnownPattern_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D117Predict8x8Kernel(acc);
            ...
            Vp9DirectionalPredictor.D117Predict(topLeft, above, left, cpuDst, 8, 8);
    ...
    [TestMethod]
    public async Task Vp9D117Predict8x8Kernel_RandomInputs_BitExact()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D117Predict8x8Kernel(acc);
            var rng = new Random(unchecked((int)0xCAFE0199u));
    ...
    [TestMethod]
    public async Task Vp9D117Predict8x8Kernel_BatchedDispatch_AllBlocksBitExact()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D117Predict8x8Kernel(acc);
            const int n = 8;
            var rng = new Random(unchecked((int)0xCAFE1990u));
