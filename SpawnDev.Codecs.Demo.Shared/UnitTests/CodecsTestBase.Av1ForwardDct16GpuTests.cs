// Tests for Av1ForwardDct16Gpu.Forward16 driven through
// Av1ForwardDct16GpuKernel. Verifies the in-kernel-callable static
// helper produces output bit-exact vs Av1ForwardDct16.Transform across
// (a) zero, (b) DC-only, (c) random batches, (d) all cos_bit values.
...
{
    [TestMethod]
    public async Task Av1ForwardDct16Gpu_ZeroInput_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Av1ForwardDct16GpuKernel(acc);
    ...
    [TestMethod]
    public async Task Av1ForwardDct16Gpu_DcOnlyInput_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
    ...
    [TestMethod]
    public async Task Av1ForwardDct16Gpu_RandomBatch_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            ...
            var rng = new Random(unchecked((int)0xA1F416BAu));
    ...
    [TestMethod]
    public async Task Av1ForwardDct16Gpu_AllCosBits_MatchCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            ...
            var rng = new Random(unchecked((int)0xFD161CAFu));
            var input = new int[transformCount * 16];
