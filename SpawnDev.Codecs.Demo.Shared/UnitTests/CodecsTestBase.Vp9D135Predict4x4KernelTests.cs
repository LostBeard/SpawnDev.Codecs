// Tests for Vp9D135Predict4x4Kernel (slice 192).
...
{
    [TestMethod]
    public async Task Vp9D135Predict4x4Kernel_KnownPattern_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D135Predict4x4Kernel(acc);
            byte[] above = { 10, 20, 30, 40 };
            byte[] left = { 50, 60, 70, 80 };
            byte topLeft = 5;

            var cpuDst = new byte[16];
            Vp9DirectionalPredictor.D135Predict(topLeft, above, left, cpuDst, 4, 4);
    ...
    [TestMethod]
    public async Task Vp9D135Predict4x4Kernel_FlatInput_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
    ...
    [TestMethod]
    public async Task Vp9D135Predict4x4Kernel_RandomInputs_BitExact()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D135Predict4x4Kernel(acc);
            var rng = new Random(unchecked((int)0xCAFE0192u));
    ...
    [TestMethod]
    public async Task Vp9D135Predict4x4Kernel_BatchedDispatch_AllBlocksBitExact()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Vp9D135Predict4x4Kernel(acc);
            const int n = 16;
            var rng = new Random(unchecked((int)0xCAFE1920u));
            ...
            for (int b = 0; b < n; b++)
            {
                Vp9DirectionalPredictor.D135Predict(
                    topLeftFlat[b],
