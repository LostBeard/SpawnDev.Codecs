// Tests for Av1ForwardDct8Kernel. Validates bit-exact match with
// Av1ForwardDct8.Transform across (a) zero, (b) DC-only / structured
// input, (c) random batches, (d) all cos_bit values.
...
{
    [TestMethod]
    public async Task Av1ForwardDct8Kernel_ZeroInput_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            using var kernel = new Av1ForwardDct8Kernel(acc);
            var input = new int[8];
            var cpuOut = new int[8];
            Av1ForwardDct8.Transform(input, cpuOut);
    ...
    [TestMethod]
    public async Task Av1ForwardDct8Kernel_DcOnlyInput_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
    ...
    [TestMethod]
    public async Task Av1ForwardDct8Kernel_RandomBatch_MatchesCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            ...
            for (int t = 0; t < transformCount; t++)
                Av1ForwardDct8.Transform(input.AsSpan(t * 8, 8), cpuOut.AsSpan(t * 8, 8));
    ...
    [TestMethod]
    public async Task Av1ForwardDct8Kernel_AllCosBits_MatchCpuReference()
    {
        var (ctx, acc) = await CreateKernelAcceleratorAsync();
        try
        {
            ...
            for (int cosBit = 10; cosBit <= 13; cosBit++)
            {
                var cpuOut = new int[transformCount * 8];
                for (int t = 0; t < transformCount; t++)
                    Av1ForwardDct8.Transform(input.AsSpan(t * 8, 8), cpuOut.AsSpan(t * 8, 8), cosBit);
