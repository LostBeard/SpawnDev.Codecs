// Tests for Vp9D135Predict16x16Kernel (slice 204).
...
{
    [TestMethod]
    public async Task Vp9D135Predict16x16Kernel_KnownPattern_MatchesCpuReference()
    {
        var (ctx, acc) = await AcquireAcceleratorOrSkipAsync();
        try
        {
            using var kernel = new Vp9D135Predict16x16Kernel(acc);
            ...
            Vp9DirectionalPredictor.D135Predict(topLeft, above, left, cpuDst, 16, 16);
    ...
    [TestMethod]
    public async Task Vp9D135Predict16x16Kernel_RandomInputs_BitExact()
    {
        var (ctx, acc) = await AcquireAcceleratorOrSkipAsync();
        try
        {
            using var kernel = new Vp9D135Predict16x16Kernel(acc);
            var rng = new Random(unchecked((int)0xCAFE0204u));
    ...
    [TestMethod]
    public async Task Vp9D135Predict16x16Kernel_BatchedDispatch_AllBlocksBitExact()
    {
        var (ctx, acc) = await AcquireAcceleratorOrSkipAsync();
        try
        {
            using var kernel = new Vp9D135Predict16x16Kernel(acc);
            const int n = 4;
            var rng = new Random(unchecked((int)0xCAFE2040u));
