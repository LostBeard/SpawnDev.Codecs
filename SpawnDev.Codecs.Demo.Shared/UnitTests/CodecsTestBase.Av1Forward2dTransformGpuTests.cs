...
// GPU-callable 2D forward DCT helpers (Tx4x4 + Tx8x8 + Tx16x16 DCT_DCT)
...
{
    [TestMethod]
    public async Task Av1Forward2dTransformGpu_4x4_ZeroInput_MatchesCpu()
    {
        ...
        try
        {
            ...
            Av1Forward2dTransform.Apply(Av1TxSize.Tx4x4, Av1TxType.DctDct, input, cpuOut);
            ...
            kernel.Run(dIn.View, dOut.View, dScratch.View, blockCount: 1, txSize: 0);
    ...
    [TestMethod]
    public async Task Av1Forward2dTransformGpu_4x4_DcOnly_MatchesCpu()
    {
        var (ctx, acc) = await AcquireAcceleratorOrSkipAsync();
        try
        {
    ...
    [TestMethod]
    public async Task Av1Forward2dTransformGpu_4x4_RandomBatch_MatchesCpu()
    {
        var (ctx, acc) = await AcquireAcceleratorOrSkipAsync();
        try
        {
            ...
            int n = 16;
            var rng = new Random(unchecked((int)0xA12404BAu));
            ...
            for (int b = 0; b < blockCount; b++)
                Av1Forward2dTransform.Apply(Av1TxSize.Tx4x4, Av1TxType.DctDct,
            ...
            dIn.View.CopyFromCPU(input);
            kernel.Run(dIn.View, dOut.View, dScratch.View, blockCount, txSize: 0);
            await acc.SynchronizeAsync();
            var readback = await SpawnDev.ILGPU.SpawnDevContextExtensions.CopyToHostAsync(dOut);
