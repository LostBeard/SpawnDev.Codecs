            ...
            foreach (var project in projects)
            {
                if (project.AppProjectType == ProjectType.BlazorWasm)
                {
                    ...
                    try
                    {
                        ...
                        // Bulk-scrape the test rows in one Page.EvaluateAsync call.
                        // The previous per-row Locator loop did 2 IPC round-trips per
                        // row, which scaled to 2-5 minutes once a project crossed
                        // ~3000 test rows. Doing the scrape in browser-side JS and
                        // returning all rows as a single JSON payload reduces the
                        // enumeration phase to sub-second on the same row counts.
                        ...
                        LogStatus($"Browser tests enumerated: {testableProject.Tests.Count} tests (from {totalRows} rows)");
                ...
                else if (project.AppProjectType == ProjectType.Exe)
                {
                    ...
                    // reference data served by the static file server.
                    // The Blazor WASM project's static server runs at port 6421 (Codecs-specific).
                    if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("TEST_SERVER_URL")))
                    {
                        Environment.SetEnvironmentVariable("TEST_SERVER_URL", "https://localhost:6421/");
                        LogStatus("Set TEST_SERVER_URL=https://localhost:6421/ for desktop tests");
                    }
                    ...
                    foreach (var test in testList)
                    {
                        ...
                        {
                            var result = await ProcessRunner.Run(publishedBinary, rowTest.Name, timeout: 300_000).ConfigureAwait(false);
