...
> **Status: 6 of 6 codec decoders WORKING (5 bit-exact + AV1 pipeline end-to-end) + 4 of 6 codec encoders WORKING as of 2026-04-27.** FLAC + Vorbis + VP8 + VP9 decode real-world data; VP8 walker output matches ffmpeg with MAE 0.04/0.01/0.00 across Y/U/V planes (loop filter accounts for the residual ~56 max diff). VP9 walker decodes BBB.webm first keyframe with first 16x16 Y BIT-EXACT and 352 leaf blocks total. FLAC + Vorbis + VP8 + **VP9** encode real-world data; ffmpeg accepts our VP8 + VP9 keyframe bitstreams (single-block VP9 frames decode pixel-perfect via ffmpeg native VP9 decoder, multi-block path internally consistent through Vp9KeyframeWalker round-trip but desyncs in ffmpeg pending a walker-side fix); ffmpeg accepts our Vorbis ogg at 440Hz exact. AV1 has parser + range coder pair + framing all bit-exact vs libaom-av1; full default CDF set landed (partition + intra mode + skip + txfm + segment + token), execution wiring (mode_info / decode_coefs / inverse transform / intra predict / reconstruct) is the next step. Opus CELT decoder is the only remaining audio gap (~2000 LoC libopus port). See the feature matrix below.
