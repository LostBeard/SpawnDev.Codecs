...
> **Status: 6 of 6 codec decoders WORKING (5 bit-exact + AV1 pipeline end-to-end) + 6 of 6 codec encoders WORKING (5 bit-exact + AV1 walker round-trip pixel-correct, libdav1d MSAC validation pending) as of 2026-04-27.** FLAC + Vorbis + VP8 + VP9 decode real-world data; VP8 walker output matches ffmpeg with MAE 0.04/0.01/0.00 across Y/U/V planes. VP9 walker decodes BBB.webm first keyframe with first 16x16 Y BIT-EXACT and 352 leaf blocks total. FLAC + Vorbis + VP8 + VP9 + **Opus** + **AV1** encode real-world data; ffmpeg accepts our VP8 + VP9 keyframe bitstreams (single-block VP9 frames decode pixel-perfect); ffmpeg accepts our Vorbis ogg at 440Hz exact; Opus encode + decode round-trip via the Concentus 2.2.2 BSD-3 backbone across mono/stereo, 8/16/24/48 kHz, 2.5 / 5 / 10 / 20 ms frames; AV1 keyframe encoder produces TD+SH+Frame OBU bitstream that ffmpeg trace_headers parses cleanly + walker round-trips pixels within 0.2 of source (libdav1d MSAC entropy gap remains for full third-party decode parity). See the feature matrix below.
