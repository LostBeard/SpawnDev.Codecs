...
> **Status: 6 of 6 codec encoders + 6 of 6 codec decoders WORKING through the public IVideoDecoder / OpusDecoder / FlacDecoder / VorbisOggDecoder APIs as of 2026-04-27.** Every audio + video encoder produces bytes that the matching reference decoder accepts: FLAC bit-exact lossless; Opus round-trip via Concentus 2.2.2 (mono/stereo, 8/16/24/48 kHz, 2.5-20ms frames); Vorbis ogg accepted by ffmpeg/libvorbis at correct amplitude (peak 0.343 vs source 0.30, RMS 0.124 vs 0.122); VP8 keyframes accepted by ffmpeg pixel-perfect; VP9 keyframes accepted by ffmpeg native decoder bit-exact across multi-block frames (32x32 max diff Y=1 U=0 V=0 at Q=8); AV1 keyframes accepted by libdav1d (16x16 flat Y=128 reconstructs to Y=128/U=128/V=128 exact). Every decoder API surface (`Vp8Decoder` / `Vp9Decoder` / `Av1Decoder`) routes real keyframes through the walker pipelines and emits real YUV planes to the sink (no `NotImplementedException`, no placeholder mid-gray). See the feature matrix below.
